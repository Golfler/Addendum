...
namespace CourseWebApi.Models
{
    public class PushNotications
    {
        ...
        public void ApplePush(string Message, string iosMessageJson, long UserID, bool IsGolfer, string pushMsgFrom, ref List<string> lstMsgs)
        {
            try
            {
                ...
                if (isPushSend)
                {
                    lstMsgs.Add("DeviceUUID: " + regapnId);
                    if (regapnId != "" && regapnId != null)
                    {
                        lstMsgs.Add("Final start.");
                        //IOS Push
                        ...
                        push.RegisterAppleService(new ApplePushChannelSettings(true, appleCert, ConfigClass.CERT_PASSWORD, true), new PushServiceSettings() { AutoScaleChannels = false, Channels = 1, MaxAutoScaleChannels = 1, MaxNotificationRequeues = 1, NotificationSendTimeout = 5000 });
                    ...
                    {
                        lstMsgs.Add("device id not found.");
                    }
                }
            }
            ...
            {
                lstMsgs.Add("Exception: " + Convert.ToString(ex.Message));
                ...
                w.WriteLine("Course PushMessage: " + ex.Message);
        ...
        {
            //Do something here
            //var test = notification.QueuedCount;
            //using (var sw = File.AppendText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, ConfigClass.ErrorFilePath)))
            //{

            //    sw.Write(DateTime.Now + " at notification failed.");
        ...
        public string getGCMID(long userid, bool isGolfer, string pushMsgFrom, ref bool isPushSend)
        {
            ...
            if (isGolfer)
            {
                var enduser = _db.GF_Golfer.FirstOrDefault(x => x.GF_ID == userid);
                if (enduser != null)
                {
