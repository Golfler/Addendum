    ...
    if ($('#tr' + id + ' .sublist').hasClass('ui-jqgrid-btable')) {
        ...
        if (!subOrgListFlag) {
            bindJqGridNoMultiSelect('sublist' + id, '', $('#getsublisturl').val(),
                ['ID', 'Sub Org Name', 'Action'
            // 'App Store Link', 'Add Credits',
            // ,
            ...
            //                    { name: 'Action', index: 'Action', sortable: false, formatter: SetSubOrgLink, align: "center" },
                    {name: 'Create', index: 'Create', sortable: false, formatter: isTanentIDForShared, align: "left" }
            //,
...
function isTanentIDForShared(cellvalue, options, rowObject) {
    try {
        var hasTenantId = rowObject["hasTenantId"];
        var orgID = rowObject["ID"];
        var tenid = rowObject["TenantId"];
        if (!hasTenantId) {
            return "TenantUUID not available."
        }
        else {
            return "<a href='javascript:void();' onclick='EnrollDevice(&apos;" + tenid + "&apos;)' >Enroll Device</a>";
        }
    }
    catch (ex) { alert(ex.message); }
}

function EnrollDevice(tenid) {
    if (confirm("Are you sure you want to enroll device?")) {
        var notifyurl = $("#hdnNotifyURL").val();
        var notifylink = notifyurl + "/enroll?tid=" + tenid;
